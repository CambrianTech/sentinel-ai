...
def verify_alloy_integrity(forged_dir: Path, alloy: dict) -> list[str]:
    """Verify alloy hashes match actual files before publishing.
    ...
    if not integrity:
        return []  # No attestation to verify — Phase 1, that's fine
        ...
        if model_dir.exists():
            ...
            if safetensors:
                ...
                if actual_hash != claimed_hash:
                    errors.append(f"MODEL HASH MISMATCH: claimed {claimed_hash[:20]}... actual {actual_hash[:20]}...")

    # Verify code attestation
    code = integrity.get("code", {})
    if code.get("binaryHash"):
        script_path = Path(__file__).parent / "scripts" / "forge_model.py"
        if script_path.exists():
            actual = f"sha256:{hashlib.sha256(script_path.read_bytes()).hexdigest()}"
            if actual != code["binaryHash"]:
                # Not an error — forge runner may have been updated since forge ran
                print(f"  NOTE: forge_model.py hash differs from attestation (runner updated since forge)")
...
def publish_model(forged_dir: Path, org: str, domain: str, dry_run: bool = False):
    ...
    # ── Alloy integrity verification (delivery phase) ──────────────
    alloy_files = list(forged_dir.glob("*.alloy.json"))
    alloy_data = None
    if alloy_files:
        alloy_data = json.load(open(alloy_files[0]))
        errors = verify_alloy_integrity(forged_dir, alloy_data)
        if errors:
            for err in errors:
                print(f"  INTEGRITY ERROR: {err}")
            if not dry_run:
                print(f"  ABORTING PUBLISH — integrity verification failed")
                return False

        # Hash the card and store in alloy's publish stage
        import hashlib
        card_hash = f"sha256:{hashlib.sha256(card.encode()).hexdigest()}"
        for stage in alloy_data.get("stages", []):
            if stage.get("type") == "publish":
                stage["cardHash"] = card_hash
                break

        # Write updated alloy back
        alloy_files[0].write_text(json.dumps(alloy_data, indent=2))
        print(f"  Alloy verified, card hash: {card_hash[:30]}...")
