"""
BLOOM Model Loader for Sentinel-AI

This module provides functions to load BLOOM models into the Sentinel-AI adaptive architecture.
It handles the initialization of the adaptive transformer from a pretrained BLOOM model.
"""
...
# Define the consistent function name expected by PruningModule
def load_bloom_with_adaptive_transformer(model_name, baseline_model, config, device, debug=False, quiet=False):
    """
    Wrapper function to maintain consistent API with other loaders
    Delegates to load_adaptive_model_bloom
    """
    return load_adaptive_model_bloom(model_name, baseline_model, config, device, debug, quiet)

# Import from the new sentinel package
try:
    from sentinel.models.adaptive.transformer import AdaptiveCausalLmWrapper
except ImportError:
    # Fall back to legacy imports if not yet available
    from models.adaptive_transformer import AdaptiveCausalLmWrapper
    warnings.warn(
        "Importing from models.adaptive_transformer is deprecated. "
        "Please use sentinel.models.adaptive.transformer instead.",
        DeprecationWarning,
        stacklevel=2
    )
