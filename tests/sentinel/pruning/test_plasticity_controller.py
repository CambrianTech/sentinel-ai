...
To run these tests:
    pytest -xvs tests/sentinel/pruning/test_plasticity_controller.py
"""
...
class TestPlasticityController:
    ...
    def test_aggressive_thresholds(self):
        """Test very aggressive thresholds to check if pruning happens."""
        # This test specifically verifies that with the aggressive thresholds,
        # we should see pruning happen with values in the expected entropy/gradient ranges
        controller = PlasticityController(
            model=None, 
            total_layers=1, 
            heads_per_layer=1,
            high_entropy_threshold=0.4,  # Very aggressive - almost any head could be pruned
            low_entropy_threshold=0.2,
            grad_threshold=1e-3
        )
        
        # Create 100 random heads with different entropy/gradient values
        np.random.seed(42)  # For reproducible tests
        heads = []
        decisions = []
        
        for _ in range(100):
            entropy = np.random.uniform(0.3, 0.7)  # Random entropy between 0.3 and 0.7
            grad_norm = np.random.uniform(0, 2e-3)  # Random grad norm between 0 and 2e-3
            head_stats = {'is_zeroed': False, 'zeroed_epochs': 0}
            decision = controller._decide_head_fate(head_stats, entropy, grad_norm)
            heads.append((entropy, grad_norm))
            decisions.append(decision)
        
        # Count decisions
        pruned_count = sum(1 for d in decisions if d == PlasticityDecision.PRUNE)
        
        # With our aggressive thresholds, we should prune a significant number of heads
        assert pruned_count > 10, f"Only {pruned_count} heads pruned with aggressive thresholds"
